...
class BBBLinearFactorial(nn.Module):
    def __init__(self, in_features, out_features, p_logvar_init=-3, p_pi=1.0, q_logvar_init=-5):
        ...
        self.normflow = NormalizingFlow(out_features, in_features)
    ...
    def probforward(self, input, MAP=False):
        ...
        w_sample, w_log_det = self.normflow(w_sample)
        b_sample, b_log_det = self.normflow(b_sample)

        kl_w = torch.sum((self.qw.logpdf(w_sample) + w_log_det) - (self.pw.logpdf(w_sample) + b_log_det))
        kl_b = torch.sum((self.qb.logpdf(b_sample) + w_log_det) - (self.pb.logpdf(b_sample) + b_log_det))
...
class NormBBBMLP(nn.Module):
    def __init__(self, in_features, num_class, num_hidden, num_layers, p_logvar_init=-3, p_pi=1.0, q_logvar_init=-5):
        ...
        super(NormBBBMLP, self).__init__()
        layers = [
